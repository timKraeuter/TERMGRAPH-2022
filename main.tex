...
% Start Event + Refer to start graph.
...
If start events have more than one outgoing sequence flow they function as an \textit{implicit parallel gateway}, forking the sequence flow (see parallel gateway in \autoref{fig:GatewayTemplates}).
...
The rule is automatically generated once during the model transformation to graph transformation systems and is used to terminate processes, sub processes, and event sub processes, if they do not contain any tokens directly or in a subprocess.
It uses negative-application conditions to forbid tokens and sub processes for a process snapshot and then changes its state from running to terminated\footnote{The terminate rule implemented in Groove is contained in the artifacts of this paper \cite{timkrauterArtifactsTERMGRAPH2022}.}.
...
\end{figure}
% Explain End events behave exactly the same
% Explain similarity to send tasks
% Explain currently only one incoming sequence flow is allowed
