...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.55\linewidth]{images/bpmn_semantics-bpmn-metamodel.pdf}
...
A BPMN process model is represented by a \textsf{Collaboration} that has a set of \textsf{participants} and \textsf{MessageFlows} between \textsf{InteractionNodes} contained by them (see  \cref{fig:bpmnMetamodel}).
...
For further information and a detailed description of typed attributed graphs with inheritance refer to \cite{heckelGraphTransformationSoftware2020}.
...
The SPO property that dangling edges (if any) are removed automatically during rule application is not exploited in our rules.
Thus, in the future one could switch to a different approach if needed.
...
Due to SPO rewriting, a graph transformation rule in this paper is given by a graph morphism $L \to R$, while $L$ and $R$ are typed attributed graphs. 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.94\textwidth]{images/bpmn_semantics-feature-overview.pdf}
...
This can be implemented using the concept of \emph{nested quantification} \cite{rensinkNestedQuantificationGraph2006}.
Concretely, we use an optional existential quantifier to send a message only if the receiving process is ready to receive it.
