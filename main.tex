...
Furthermore, we consider allowing the user to define a start graph similar to how he could define atomic propositions for custom properties (see \cref{subsec:customProperties}).
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.55\textwidth]{images/bpmn_semantics-task-rules.pdf}
...
The message throw event rule moves the token through the event and sends a message to a waiting process snapshot, which has a token waiting at the corresponding message receive event.
...
We use the graph-transformation tool Groove\footnote{\url{https://groove.ewi.utwente.nl/about}} in our implementation since it supports all the theoretical constructs that we need.
% Evaluation
...
% Tool
In addition, we implemented most of the presented concepts in a web-based tool to make our ideas easily accessible to other researchers and potentially practitioners in the future.
