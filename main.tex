...
An example of a start graph is shown \cref{fig:startGraph} using abstract syntax (\cref{fig:startGraphAbstract} and concrete snytax (\cref{fig:startGraphConcrete}).
...
The rule in \cref{fig:taskRules} represents the start of the task, which will move one token from any incoming sequence flow to the task itself.
