...
Many types of \textsf{Activities}, \textsf{Gateways}, and \textsf{Events} exist, such as start and end events, for example, used in \cref{fig:approach}.
...
To concisely depict graphs conforming to the type graph, we introduce a concrete syntax in the clouds attached to the elements.
It extends the BPMN syntax by adding tokens.
Tokens are represented as colored circles drawn at their specified position in a model.
...
Afterward, we will explain how these and more rules are created by our model transformation.
...
The start event rule is generated by the rule template given in \autoref{fig:startAndEndTemplate} later.
...
This rule is generated by the rule template given in \autoref{fig:activityTemplates} later.
...
In the following subsections, we use this concrete syntax to describe how these rules and rules for other flow nodes are generated.
The generation is depicted using rule generation templates that describe how specific rules are created for varying flow nodes.
We explain rule generation for process instantiation and termination, activities and subprocesses, gateways, and message events.
...
Process termination is implemented with a generic rule applicable to all process snapshots.
The rule is automatically generated once during the model transformation to graph transformation systems.
It is used to terminate processes, subprocesses, and event subprocesses if they do not have any tokens and subprocesses.
The rule uses negative-application conditions to forbid tokens and subprocesses for a process snapshot and then changes its state from running to terminated.
...
\autoref{fig:startAndEndTemplate} depicts the rule generation templates for activities and subprocesses.
...
It consumes a token at the activity and creates one token at each outgoing sequence flow.
...
By consuming an incoming token, a subprocess with a token at each start event is created.
To depict the process-subprocess relation, we chose the same color for the outer ring of the subprocess snapshot and its tokens.
...
The bottom rule template generates one rule to consume a terminated subprocess and create tokens at each outgoing sequence flow.
Subprocesses are terminated by a generic rule if they neither have tokens nor subprocesses (see section \ref{subsec:instAndTermination}).
...
A message throw event consumes an incoming token and adds a token at each outgoing sequence flow.
...
The upper rule template shows that message intermediate catch events consume one incoming message and token and add a token at each outgoing sequence flow.
In the lower rule template, one can see that receive tasks rules are similar to the activity rules in \autoref{fig:activityTemplates}, besides consuming a message when a receive task is started.
...
Like other work, we differentiate between \emph{BPMN-specific properties} defined for all BPMN process models and \emph{custom properties} tailored towards a particular BPMN process model.
...
A key difference is that the rules in \cite{vangorpVisualTokenbasedFormalization2013} are general and can be applied to any BPMN model, while we generate specific rules for every BPMN model based on rule templates.
Thus, our approach can be seen as a partial evaluation compared to \cite{vangorpVisualTokenbasedFormalization2013} since we process a concrete BPMN model before its execution.
...
Using these semantics, BProVe can verify custom LTL properties and BPMN-specific properties, such as Safeness and Soundness.
...
% Tool
In addition, we provide a prototype implementation in a web-based tool to make our ideas easily accessible to other researchers and potential practitioners in the future.
