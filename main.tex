...
Furthermore, the tool is easily accessible online and includes checking of BPMN-specific properties, such as safeness and soundness \cite{corradiniBProVeToolSupport2017, corradiniFormalApproachAnalysis2021}.
...
\begin{table}[htbp]
    ...
    \hline
     ...
     Boundary Events & X\footnote{Only supports interrupting boundary events on tasks.} & & X\footnote{Only supports message and timer events.} & X\\ % To the same extent as the event support
      ...
      Start Events &  & & & X\footnote{\label{footnote:startEventRestriction}We support signal and message start events.}\\
      Start Events (non-interrupting) & & & & X\cref{footnote:startEventRestriction}\\
    \end{tabular}
\end{table}
