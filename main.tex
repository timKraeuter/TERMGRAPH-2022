...
To achieve this, the rule uses negative-application conditions to forbid tokens and subprocesses for a process snapshot and then changes its state from running to terminated.
...
In the particular case that there is no start event in the subprocess, a token for every activity and gateway without incoming sequence flows is created.
