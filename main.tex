...
Afterwards, we will explain how these and more rules are created by our model transformation.
...
Now we have described four example rules and introduced a concrete syntax to depict them concisely and understandably.
In the following subsections, we will use this concrete syntax to describe how these and rules for other flow nodes are generated.
The generation is depicted using rule generation templates that describe how specific rule are created for varying flow nodes.
...
It is used to terminate processes, sub processes, and event sub processes, if they do not have any tokens and subprocesses.
To achieve this, the rule uses negative-application conditions to forbid tokens and sub processes for a process snapshot and then changes its state from running to terminated.
