...
% We are doing an application thus we explain the application area and the applied theory.
\subsection{BPMN}
% Introduce BPMN and the BPMN semantics roughly. Especially token distributions.
% Could move figure 3b here if wanted.

\subsection{Typed attributed graphs and graph transformations}
...
In addition, we are using inheritance in the type graph.
For further information and a detailed description of typed attributed graphs with inheritance refer to \cite{heckelGraphTransformationSoftware2020}. % Better reference? There are no formal definitions here.

% Groove uses the single pushout approach with negative application conditions.
Regarding graph transformation, we utilize the single pushout approach with negative application conditions (NAC) \cite{ehrigALGEBRAICAPPROACHESGRAPH1997}.
We need NACs to implement some more intricate parts in the BPMN execution semantics such as the termination of processes. 
Thus, a graph transformation rule is a graph morphism $L \to R$, while $L$ and $R$ are typed attributed graphs. 
% We chose the single pushout approach because? Why does it fit us? Dangling condition?
% What is van gorp using? What are the alternatives? What about DPO?
% Change the description of figure 5 or even the figure itself.
\section{BPMN Semantics formalization} \label{sec:formalization}

Since our approach is based on a model transformation from BPMN to graph transformation systems, we generate a \emph{start graph} and \emph{graph transformation rules} for a given BPMN process model (see \cref{fig:bpmnMetamodel}).
The approach supports the BPMN constructs depicted in \cref{fig:bpmnConstructsOverview}.

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{images/bpmn_semantics-feature-overview.pdf}
...
\begin{figure}[h]
    ...
    \caption{Example start graph in abstract and concrete syntax}
...
\Cref{fig:gtRule} depicts an example graph transformation rule for a NSE in abstract and concrete syntax.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.38\textwidth]{images/bpmn_semantics-atomic-proposition.pdf}
